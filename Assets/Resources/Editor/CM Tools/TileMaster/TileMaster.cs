...
public class TileMaster : EditorWindow {
	...
	static bool editorEnabled = true;
	...
	static void ResetManager()
	{
		...
		if(cmquad == null)
		{
			...
			//EditorUtility.SetSelectedRenderState(cmquad.GetComponent<Renderer>(), EditorSelectedRenderState.Hidden);
...
//					Debug.Log("Tilecount: " + (int)(curTileSet.width/gridSizeX) + ", " + (int)(curTileSet.height/gridSizeY));
		...
		EditorGUILayout.BeginHorizontal();
		EditorGUILayout.LabelField("Editor Enabled", GUILayout.Width(150));
		editorEnabled = EditorGUILayout.Toggle(editorEnabled);
		EditorGUILayout.EndHorizontal();
	}
